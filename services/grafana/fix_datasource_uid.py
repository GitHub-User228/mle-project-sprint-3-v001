...
username = os.getenv(
    "GRAFANA_USER"
)  # Вставить user, с помощью которого логинимся в web
password = os.getenv(
    "GRAFANA_PASS"
)  # Вставить pass, с помощью которого логинимся в web
...
if not current_uid:
    print(
        "Warning: 'uid' not found in the datasource response. Using empty string."
    )
